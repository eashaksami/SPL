...
void bandPassFilter(vector<double> arr)
{
    double maxFrequency,minFrequency;
    cout<<"enter min and max frequency"<<endl;

    cin>>minFrequency>>maxFrequency;
    for(int i=0;i<arr.size();i++)
    {
        if(arr[i]>maxFrequency || arr[i]<minFrequency)
        {
...
{
    //Frequency array
    double arr[] = {100.0,60.5,50.0,70.5,90.3,110.2,150.7,86.9,
